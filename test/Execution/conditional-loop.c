// RUN: clang -fclangir -emit-cir %S/../Integration/conditional-loop.c -o %t.cir
...
extern float conditional_sum(float x);
...
int main(void) {
  ...
  for (unsigned i = 0; i < sizeof(inputs) / sizeof(inputs[0]); ++i) {
    float expected = 2.0f * inputs[i];
    float actual = conditional_sum(inputs[i]);
    if (actual != expected) {
